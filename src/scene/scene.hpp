...
class AbsScene {
   private:
    std::vector<mobject::mobject_ptr> mobjects_;

   public:
    virtual mobject::MObject* add(mobject::mobject_ptr obj) {
        return mobjects_.emplace_back(std::move(obj)).get();
    }
    virtual mobject::MObject* add(mobject::MObject* obj) {
    ...
    std::vector<mobject::mobject_ptr>& mobjects() { return mobjects_; }
};
}  // namespace xcal::scene